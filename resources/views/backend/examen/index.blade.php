...
<th>Type exament</th>
<th>Nom patient </th>
<th>Date d'exament</th>
<th>Action</th>
</tr>
</thead>

<tbody>
@foreach($exament as $agent)
    
<tr>
<td>{{$agent->agentt->nom}}</td>
<td>{{$agent->type_exament->type_exament}}</td>
...
<a class="dropdown-item" href="{{route('createResultat',$agent->id)}}"><i class="icofont icofont-tasks-alt"></i>Resultat</a>
<a class="dropdown-item" href="{{route('resultats',$agent->id)}}"><i class="icofont icofont-tasks-alt"></i>voir le resultat</a>
